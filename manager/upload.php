    ...
    if(!empty($_POST['password'])&&!empty($_FILES['file'])){
        if($_POST['password']=='zy9296051'){
            ...
            && in_array($extension, $allowedExts))
            {
                ...
                {
                    echo json_encode(array('status'=>'error','info'=>$_FILES["file"]["error"]));
                }
                else
                {
                ...
                echo json_encode(array('status'=>'error','info'=>'非法文件格式'));
            }
        }
        else{
            echo json_encode(array('status'=>'error','info'=>'密码错误'));
        }
    }
}
?>